...
RenderLayer::RenderLayer(OGRLayer *layer, int indexGrid)
  ...
  boundary.Merge(boundary.MinX - boundaryWidth / 100,
                 boundary.MinY - boundaryHeight / 100);
  boundary.Merge(boundary.MaxX + boundaryWidth / 100,
                 boundary.MaxY + boundaryHeight / 100);

  boundaryQ.setCoords(boundary.MinX, boundary.MinY, boundary.MaxX,
                      boundary.MaxY);
...
void RenderLayer::renderInit(QOpenGLShaderProgram *shader) {
  ...
  vboOrigin.allocate(vertexVector.constData(),
                     vertexVector.size() * sizeof(float));
  ...
  if (geoType == 2) {
    ...
    vboTriangular.allocate(triResult.constData(),
                           triResult.size() * sizeof(float));
    shader->enableAttributeArray(0);
    shader->setAttributeBuffer(0, GL_FLOAT, 0, 2, 2 * sizeof(float));
  }
  ...
  vboRTree.allocate(RTreeVertex.constData(),
                    RTreeVertex.size() * sizeof(float));
  shader->enableAttributeArray(0);
  shader->setAttributeBuffer(0, GL_FLOAT, 0, 2, 2 * sizeof(float));
}
...
void RenderLayer::render() {
  ...
  if (geoType == 2) {
    // 先画面，再画线，不然被覆盖
    vaoTriangular.bind();
    shader->setUniformValue(shader->uniformLocation("ourColor"), .0f, .8f, .0f,
                            1.0f);
    for (int i = 0; i < triIndex.size() / 2; i++) {
      glFuncs->glDrawArrays(GL_TRIANGLE_STRIP, triIndex.at(2 * i),
                            triIndex.at(2 * i + 1));
    }
  }
  ...
  vaoOrigin.bind();
  shader->setUniformValue(shader->uniformLocation("ourColor"), .8f, .8f, .8f,
                          1.0f);
  ...
  if (geoType == 0) {
    ...
    for (int i = 0; i < vertexIndex.size() / 2; i++) {
      glFuncs->glDrawArrays(GL_LINE_STRIP, vertexIndex.at(2 * i),
                            vertexIndex.at(2 * i + 1));
    }
  }
  ...
  if (selectVertex.size() != 0) {
    ...
    // 需要绑定VBO才能重新分配内存！！！
    vboSelect.bind();
    vboSelect.allocate(selectVertex.constData(),
                       selectVertex.size() * sizeof(float));

    shader->setUniformValue(shader->uniformLocation("ourColor"), .0f, .5f, .1f,
                            1.0f);
    ...
    if (geoType == 0) {
      ...
      for (int i = 0; i < selectVertexIndex.size() / 2; i++) {
        glFuncs->glDrawArrays(GL_LINE_STRIP, selectVertexIndex.at(2 * i),
                              selectVertexIndex.at(2 * i + 1));
      }
    }
  }
  ...
  if (densityVertex.size() != 0) {
    vaoDensity.bind();
    vboDensity.bind();
    vboDensity.allocate(densityVertex.constData(),
                        densityVertex.size() * sizeof(float));
...
void RenderLayer::triangulate() {
  ...
  for (int i = 0; i < vertexIndex.size() / 2; i++) {
    ...
    qDebug() << "The" << i << "triangulation finished"
             << "triResult size" << triResult.size();
  }
...
void RenderLayer::calculateDensity(float radius, float resolution) {
  ...
  for (int i = 0; i < heightPiexl; i++) {
    for (int j = 0; j < widthPiexl; j++) {
      ...
      for (int k = 0; k < features.size(); k++) {
        ...
        float distance = sqrt(pow((pointR.getX() - pointF->getX()), 2) +
  ...
  for (int i = 0; i < heightPiexl; i++) {
    for (int j = 0; j < widthPiexl; j++) {
      reverse.append(densityData.at((heightPiexl - i - 1) * widthPiexl + j));
    }
  }
  // 导出tiff
  exportDensity(boundary.MinX - radius, boundary.MaxY + radius, disPiexl,
                disPiexl, widthPiexl, heightPiexl, reverse);
...
void RenderLayer::exportDensity(float x, float y, float dx, float dy,
                                int xCount, int yCount, QVector<float> data) {
  // 定义地理信息
  ...
  poDstDS = poDriver->Create("output.tiff", xCount + 100, yCount, 1,
  ...
  band->RasterIO(GF_Write, 0, 0, xCount, yCount, (void *)data.constData(),
                 xCount, yCount, GDT_Float32, 0, 0);
  // 关闭文件
  GDALClose((GDALDatasetH)poDstDS);
}

int RenderLayer::appendFeature(OGRFeature *feature,
  ...
  if (featureType == wkbPoint) {
    ...
    if (vertexIndex.size() == 0) {
      ...
      int pointCount = vertexIndex.at(vertexIndex.size() - 2) +
                       vertexIndex.at(vertexIndex.size() - 1);
