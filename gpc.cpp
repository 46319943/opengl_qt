...
void gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
  ...
  if (subj == result) gpc_free_polygon(subj);
  if (clip == result) gpc_free_polygon(clip);

  /* Invert clip polygon for difference operation */
  if (op == GPC_DIFF) parity[CLIP] = RIGHT;
      ...
      for (intersect = it; intersect; intersect = intersect->next) {
        ...
        if (e0->bstate[ABOVE] == BUNDLE_HEAD) {
          ...
          while (search) {
            ...
            if (prev_edge) {
              if (prev_edge->bstate[ABOVE] != BUNDLE_TAIL) search = FALSE;
            } else
              search = FALSE;
          }
        }
